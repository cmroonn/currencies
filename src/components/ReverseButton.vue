...
function reverse (i) {
  const elem = this.currencyData[i];
  const currentVal = elem.Value;
  const currentNominal = elem.Nominal;

  elem.Value = elem.reverse.value;
  elem.Nominal = elem.reverse.nominal;

  elem.reverse.value = currentVal;
  elem.reverse.nominal = currentNominal;
}

</script>

<template>
  <button type="button" @click="reverse(index)">
